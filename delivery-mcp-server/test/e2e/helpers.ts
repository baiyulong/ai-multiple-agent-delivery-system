...
/** 实现记录：满足门禁的完整内容（implementation 阶段交付物） */
export function validImplementationRecord(): string {
  return [
    '# 实现记录',
    '',
    '## 1. 实现范围',
    ...
    '',
    '## 2. 代码变更清单',
    '- backend/.../CategoryController.java：新增，分类接口',
    '- frontend/src/views/supplier/category：新增，分类管理页面',
    '',
    '## 3. 工程计划符合度',
    'API 与模块结构完全按工程计划实现。',
    '',
    ...
    '',
    '## 8. 给 QA 的测试输入',
    '测试环境需先执行建表脚本；停用规则测试见工程计划第 5 节接口。',
  ].join('\n');
}
